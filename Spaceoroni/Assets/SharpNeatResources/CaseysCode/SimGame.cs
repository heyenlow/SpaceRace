...
public class SimGame //: MonoBehaviour
{
    ...
    public IEnumerator GetLegalTransitions()
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            for (int j = 0; j < builds.Count; j++)
            {
                ...
                HardNeatPlayer.Transition tmp = new HardNeatPlayer.Transition(CurrentPlayer.Builder1.Location, Coordinate.stringToCoord(moves[i]), Coordinate.stringToCoord(builds[j]), Hash);
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            for (int j = 0; j < builds.Count; j++)
            {
                HardNeatPlayer.Transition tmp = new HardNeatPlayer.Transition(CurrentPlayer.Builder2.Location, Coordinate.stringToCoord(moves[i]), Coordinate.stringToCoord(builds[j]), Hash);
                // Hash Values are currently this current game's state hash
                // need to change it to be the hash value of a game resulting from the move taking place.  without actually making the move?
    ...
    public void Transition(HardNeatPlayer.Transition t)
    {
        ...
        bool isWin = false;
        if (!Equals(Coordinate.stringToCoord(CurrentPlayer.getBuilderLocations().Substring(0, 2)), t.Builder) && !Equals(CurrentPlayer.Builder2.Location, t.Builder)) throw new System.ArgumentOutOfRangeException();
    ...
    public IEnumerator Rollout(Game game)
    {
        ...
        for (; winner == null; moveNum++)
        {
            ...
            var allPossibleMoves = new List<HardNeatPlayer.Transition>();

            HardNeatPlayer.CoroutineWithData co_data = new HardNeatPlayer.CoroutineWithData(game, GetLegalTransitions());
